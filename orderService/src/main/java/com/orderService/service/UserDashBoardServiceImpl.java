...
public class UserDashBoardServiceImpl implements UserDashBoardService {
	...
	@Autowired
	CartFeign cartServiceFeign;
	@Autowired
	InventoryFeign inventoryServiceFeign;
	@Autowired
	ProductFeign productEntityClient;
	...
	public OrdersEntity placeAOrder(String userName, OrdersEntity entity) {
		...
		List<AddressEntity> addressEntity = profileFeign.getMyAddress(userName);
		...
		MyCartLists myCartLists = cartServiceFeign.getCart(userName);
		...
		for (int i = 0; i < cartList.size(); i++) {
			...
			if (remainingQty == 0) {
				ProductsEntity prodEntity = productEntityClient.getProdById(orderProductId);
	...
	public void geratePdf(HttpServletResponse response, String orderId) throws DocumentException, IOException {
		...
		RegistrationEntity registrationEntity = profileFeign.getRegistrationEntity(ordersEntityList.getUserName());

		List<AddressEntity> listOfAddressEntity = profileFeign.getMyAddress(ordersEntityList.getUserName());
		...
		MyCartLists myCartLists = cartServiceFeign.getCart(ordersEntityList.getUserName());
