...
import com.deliveryBoyService.dao.D_AgentDao;
import com.deliveryBoyService.dao.DeliveryBoyDao;
import com.deliveryBoyService.dao.DeliveryBoyLoginDao;
import com.deliveryBoyService.dao.RegistrationDao;
import com.deliveryBoyService.dao.ReturnedOrdersDao;
import com.deliveryBoyService.entity.ReturnedOrders;
import com.deliveryBoyService.exception.CustomException;
import com.deliveryBoyService.feign.DAgentFeign;
...
//	private static Logger logger = Logger.getLogger(DeliveryBoyLoginServiceImpl.class);
	...
	public boolean sendotp(String phoneNumber) throws ParseException {
		...
		if (dAgentFeign.checkPhn(phoneNumber)) {
			// generating Otp
			int min = 100000;
			int max = 999999;
	...
	//	logger.info("Start of send method..");
		try {
			...
			MessageCreator creater = Message.creator(new PhoneNumber(phoneNumber), new PhoneNumber(FROM_NUMBER), msg);
			creater.create();
		} catch (Exception ex) {
	...
	public void sendingOtp(String phoneNumber, String generatedotp) {
		...
		//	logger.info("Start of send method..");
			...
			MessageCreator creater = Message.creator(new PhoneNumber(phoneNumber), new PhoneNumber(FROM_NUMBER),
					message);
			creater.create();
		} catch (Exception ex) {
			throw new CustomException("Unable to send Otp");
	...
	public ReturnedOrders cancledorder(ReturnedOrders returnOrders) {
		...
		String body1 = "hi this orderid  " + returnOrders.getOrderid() + "  returned due to "
				+ returnOrders.getReason();
