...
import com.profileservice.dao.PackingAgentRegistrationDao;
...
import com.profileservice.entity.ChangePasswordEntity;
 ...
 // private static Logger logger = Logger.getLogger(PackingAgentRegistrationServiceImpl.class);
	...
	public PackingAgentRegistrationEntity saveUser(PackingAgentRegistrationDto packingAgentRegistrationDto) {
		...
		if (!validateOTP(packingAgentRegistrationDto.getPhoneNumber(), packingAgentRegistrationDto.getUserOtp())) {
			...
			packing.setConfirmPassword(passwordEncoder.encode(packing.getConfirmPassword()));
	...
	{
		if(!(packingAgentRegistrationEntity.getPassword().equals(packingAgentRegistrationEntity.getConfirmPassword())))
		{
			throw new InvalidEnteredPassword("Password and confirm password must be match");
		}
		packingAgentRegistrationEntity.setPassword(passwordEncoder.encode(packingAgentRegistrationEntity.getPassword()));
		packingAgentRegistrationEntity.setConfirmPassword(passwordEncoder.encode(packingAgentRegistrationEntity.getConfirmPassword()));
